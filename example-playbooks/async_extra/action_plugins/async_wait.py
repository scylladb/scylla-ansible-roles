...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if not jid and not alias and not job:
            raise AnsibleError("one of [jid, alias, job] is required")

        if not retries:
            raise AnsibleError("retries is required")

        if not delay:
            raise AnsibleError("delay is required")
        ...
        def wait_until(x):
            if x['started'] != 1:
                raise AnsibleError("job did not start")
            return x['finished'] == 1
