...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if not alias:
            raise AnsibleError("alias is required")

        if 'async' not in self._task.args:
            raise AnsibleError("async is required")
        ...
        if running['started'] != 1:
            ...
            result.update(obj)
            self.reg_async_task(obj['ansible_job_id'], alias, task_vars)
        else:
        ...
        if retries or delay:
            if poll:
                raise AnsibleError("poll is not supported together with retries or delay")
            ...
            # TODO: poll wait support
            raise AnsibleError("poll is not supported yet")
    ...
    def v2_on_result(self, host, task, result):
        ...
        if 'failed' in result and result['failed']:
            ...
            raise AnsibleError("subtask failed: " + task.name)
